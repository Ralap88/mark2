...
cap = []
cap.append(cv2.VideoCapture('avi/0416_1200-180.avi'))
...
# set desk_free[n] to false if touch rect
def check_desk(cam, rect):
    rect = (rect[0],rect[1],rect[0]+rect[2],rect[1]+rect[3])
    for key in data.desk[cam]:
        u = iom(rect, data.desk[cam][key][0])
        if u > 0.1: desk_free[key-1] = False

# return true if desk empty
def check_desk_empty(img1,img2,cut):
    x0,y0,x1,y1=cut[0];

    img1=img1[y0:y1,x0:x1]; img2=img2[y0:y1,x0:x1]
    mask=np.zeros(img1.shape,dtype=np.uint8)

    cv2.fillPoly(mask,np.array([cut[1:]]),[255])
    img1=cv2.Canny(cv2.bitwise_and(img1,mask),100,300)
    img2=cv2.Canny(cv2.bitwise_and(img2,mask),100,300)
...
def gen_one_frame():
    ...
    # seat box display
    for cam in range(6):
        for x in data.seat[cam]:
            x1=x[1]*19//120+38+308*cam; y1=x[2]*19//120+44
            x2=x[3]*19//120+38+308*cam; y2=x[4]*19//120+44
            c = (0,0,255) if seat_empty[x[0]-1] else (0,255,255)
...
def update_30():
    # desk free check
    for n in range(103): desk_free[n] = True
    for cam in range(6):
        for x in data.box[cam][frame_cnt]: check_desk(cam,x)

    # seat empty check
    for n in range(103):
        seat_empty[n] = seat_counter[n] < 15
        seat_counter[n] = 0
        if desk_free[n]: desk_empty[n] = True
    ...
    for cam in range(6):
        ...
        for n in range(103):
            ...
            if not desk_free[n]: continue
            if n+1 not in data.desk[cam]: continue
            if not check_desk_empty(img1,img2,data.desk[cam][n+1]): desk_empty[n] = False
